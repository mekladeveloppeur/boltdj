...
// GET /api/config/maps-key — returns Google Maps key for frontend
...
router.get('/restaurants', (req, res) => {
  ...
  let q = "SELECT id,code,name,description,category,quartier,logo_emoji,image_url,delivery_time,delivery_fee,min_order,rating,total_reviews,is_open FROM restaurants WHERE status='active'";
  const params = [];
...
router.get('/restaurants/:id', (req, res) => {
  const rest = db.prepare("SELECT id,code,name,description,category,address,quartier,logo_emoji,image_url,delivery_time,delivery_fee,min_order,rating,total_reviews,is_open FROM restaurants WHERE id=? AND status='active'").get(req.params.id);
